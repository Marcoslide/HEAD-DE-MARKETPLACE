...
| 5 · Execução | ✅ **fundada** · +Central (S09) +Compliance (S10) +[Conexões reais e piloto](docs/live-connections-and-pilot.md) (S10.A) +[Crescimento](docs/growth-commercial-engine.md) (S10.B) +[Head Intelligence OS](docs/rid-head-intelligence-os.md) (S10.C) | [`mos/`](mos/README.md) — backend DDD/EDA/Clean, banco (33 entidades), API REST+OpenAPI, Collector, WhatsApp, publicação e experimentação + [**Central de Marketplace**](docs/marketplace-central.md): 4 conectores **estruturados e validados por fixtures** (integração real pendente de credencial + transporte oficial), tokens cifrados, sync com watermarks, eventos idempotentes, sinais → EPE, READ_ONLY absoluto (execuções derivadas são INTERNAS ao Head) |
| 6 · Experiência | ✅ **fundada** · +Operação (S09.A) +Catálogo no v4 (S10) +Crescimento no v6 (S10.B) | `docs/01–05` + `design/prototipo-v7` (**a experiência principal**: Home · Operação · Catálogo · **Crescimento** · Conexões · A Missão · Silêncio · Conhecimento) + [**Head Chat**](docs/head-conversational-operating-intelligence.md): a área Operação responde qualquer pergunta operacional na hora — intenção + consulta tipada + fonte declarada, mesma camada UMD em Node e navegador (fixtures agora, Central depois) |
