...
(function () {
  ...
  /* Crescimento (Sprint 10.B): leads, afiliados, promoções — dados demo rotulados */
  const growthAdapter = HEADGROWTH.createGrowthAdapter(
    HEADGROWTH.createDemoGrowthDataset(clock), clock);
  const chat = HEADCHAT.createHeadChat({ clock, mie: mieLike,
    compliance: complianceAdapter, growth: growthAdapter });
  ...
  function addHead(r, { instant = false } = {}) {
    ...
    /* Crescimento (10.B): consultas comerciais abrem a mesa certa */
    if (r.facts && /^(LEADS_|FOLLOWUPS|AFFILIATE_|PROMOTION_|CATALOG_GAP)/.test(r.facts.kind || ""))
      html += `<button class="op-link" onclick="go('crescimento')">abrir no Crescimento →</button>`;
    /* ação em massa: confirmação cria o job interno (MESMO motor da tela) */
    if (r.facts && r.facts.kind === "GROWTH_ACTION_PLAN" && r.facts.requiresConfirmation)
      html += `<button class="op-link" onclick="Crescimento.confirmFromChat('${(r.facts.action && r.facts.action.targetPlatform) || "shopee"}', ${(r.facts.items || []).length})">confirmar — criar rascunhos internos →</button>`;
